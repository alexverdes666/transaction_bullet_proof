...
1. `web` verifies the **session cookie** (httpOnly, Secure, SameSite=strict, signed).
...
> Matching by unique amount works for low volume; the schema also supports per-order
> derived deposit addresses (HD wallet) for exact attribution at scale — see `orders` model.
...
- Sessions: server-side records + signed httpOnly Secure cookies; rotation on login;
  expiry + revocation.
- **CSRF**: SameSite=strict cookies + origin checks on state-changing routes.
- **Rate limiting** on auth + scan + payment endpoints (per-IP and per-account).
